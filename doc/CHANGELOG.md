...
- **GPIO 边沿检测兼容性**：修复 `DipSwitch2Bit` 和 `Tact` 在较新内核上 `add_event_detect` 抛出 `RuntimeError: Failed to add edge detection` 的问题。根因是 v2.2.0 新增的 `remove_event_detect` 调用干扰了 RPi.GPIO 内部状态机 — `GPIO.cleanup()` 已清空所有事件检测，之后再调 `remove_event_detect` 反而使引脚进入不一致状态。现已移除多余的 `remove_event_detect`，恢复为与老版本一致的直接 `add_event_detect` 方式
- **GPIO 边沿检测降级轮询**：`DipSwitch2Bit` 和 `Tact` 在 `add_event_detect` 失败时自动降级为 GPIO 轮询模式，`is_on` 属性和 `poll()` 方法直接读取引脚电平，确保程序不会因硬件中断不可用而崩溃
- **初始化错误信息可见性**：`SAKSHAT.__init__` 中 `from None` 改为 `from e`，初始化失败时不再吞掉底层异常，便于定位问题根因
- **部分初始化后 GPIO 泄漏**：`SAKSHAT` 新增 `_gpio_done` 标志位追踪 GPIO 初始化状态，即使外设创建失败也能正确释放 GPIO 资源
- **GPIO 清理警告消除**：`_gpio_init()` 中 `setwarnings(False)` 提前到 `cleanup()` 之前，避免首次运行时出现 "No channels have been set up yet" 的 RuntimeWarning
- **GPIO 抽象层完善**：`_gpio.py` 的 `GPIOProvider` 协议和 `_MockGPIO` 新增 `remove_event_detect` 方法
